...
Open `http://<docker-host>:8080`. Play audio uses WebRTC from MediaMTX on port 8889 (WHEP `/scanner/whep`). Listen-only (VLC/MPlayer): `http://<docker-host>:8888/player/index.m3u8`. MediaMTX image default is `bluenviron/mediamtx:1-ffmpeg`: it pulls the radio on path `raw`, then ffmpeg gates scan hiss and compresses speech onto `scanner` (G.711 for WebRTC) and `player` (AAC listen-only HLS). Tune `AUDIO_GATE_THRESHOLD` in `.env` (lower if speech is chopped; raise if hiss remains). Do not run Siren, ProScan, or RH-536HP at the same time.
